...
<div class="nav">
    <div class="navBg d-flex justify-content-bewteen">
        ...
        <div :class="{'showMenu':isClickToggle}" class="d-flex menu">
            <router-link :to="item.url" @click="startScrollBar();isClickToggle = false;" v-for="(item, index) in linkList" :key="index" class="link">{{item.txt}}</router-link>
            <a v-if="!isLoginSuccess" @click="isClickToggle = false;clickLoginFn();" class="loginBtn transition-0-3">會員登入</a>
            <a @click="isClickToggle = false;clickAdminFn();goTop();" v-if="isLoginSuccess" class="loginBtn userBtn flex-direction-col transition-0-3">
                <p class="icon-cog"></p>
                <p v-if="!isAdiminsLogin">會員系統</p>
                <p v-else>管理員系統</p>
            </a>
        </div>
        <div @click="isClickToggle = !isClickToggle;clickMenuToggleFn()" :class="{'active':isClickToggle}" class="menu-toggle ">
    ...
    <div class="navBottom">
        <a v-if="!isLoginSuccess" @click="isClickToggle = false;clickLoginFn()" class="bottom-btn link loginBtn2 transition-0-3">
            <p class="icon-user">會員登入</p>
        </a>
        <a @click="isClickToggle = false;clickAdminFn();goTop();" v-if="isLoginSuccess" class="bottom-btn link loginBtn2 transition-0-3">
