<template>
    <div class="pagination">
        <div @click="clickAddNum(-1)" :class="{notClick:this.nowNum === 1}" class="d-flex prev-btn">
            <p class="icon icon-left-open"></p>
            <p class="txt">PREV</p>
        </div>
        <div class="d-flex nums">
            <p @click="clickNumBtn(num)" :class="{active:this.nowNum === num}" v-for="(num, index) in getListNum" :key="index">{{num}}</p>
        </div>
        <div @click="clickAddNum(1)" :class="{notClick:this.nowNum === this.getListNum}" class="d-flex next-btn">
            <p class="txt">NEXT</p>
